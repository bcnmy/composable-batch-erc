...
export function App() {
  ...
  // Fallback price for preview when not connected
  const displayPrice = ethPrice > 0 ? ethPrice : 3000
  ...
  // Use a default preview amount when user hasn't entered anything (for non-connected state)
  const previewAmount = inputAmount > 0 ? inputAmount : (isConnected ? 0 : 1.0)
  ...
  return (
    <div className="min-h-screen bg-surface text-text">
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        {/* Account panel — only when connected */}
        ...
        {isConnected && chain && accountLoading && (
          <div className="text-center py-4 mb-4">
            ...
            <span className="text-sm text-text-tertiary ml-2">Initializing account...</span>
          </div>
        )}

        {isConnected && !chain && (
          <p className="text-center text-danger py-8 mb-4">
            Switch to Base, Arbitrum, Optimism, or Ethereum
          </p>
        )}

        {/* Tabs — show when connected with account */}
        {isConnected && smartAccountAddress && (
          <div className="flex gap-0 border-b border-border-dim mb-4">
        ...
        {/* Trade tab — ALWAYS visible (even when not connected) */}
        {(tab === 'trade' || !isConnected) && (
          <div className="bg-surface-raised border border-border-dim rounded-lg p-5 space-y-5">
            {/* Amount input */}
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-text-secondary">Amount</span>
                {isConnected && hasUsableBalance && maxEth > 0 && (
                  <button
              ...
              <div className="flex items-center border border-border rounded-lg overflow-hidden focus-within:border-accent/50 transition">
                <input
                  ...
                  step="0.001"
                  min="0"
                  max={isConnected ? maxEth : undefined}
                  value={amountStr}
                  onChange={e => setAmountStr(e.target.value)}
                  placeholder={isConnected ? '0.0' : '1.0'}
                  disabled={isWorking}
            ...
            {previewAmount > 0 && (
              <>
                ...
                <PositionPreview
                  ethAmount={previewAmount}
                  loops={loops}
                  ethPrice={displayPrice}
                  leverage={leverage}
            ...
            {/* Fee estimate — only when connected */}
            {isConnected && feeEstimate.tokenAmount && (
            ...
            {isConnected && feeEstimate.isLoading && !feeEstimate.tokenAmount && inputAmount > 0.0001 && (
              <div className="flex items-center justify-between text-sm text-text-tertiary">
                <span>Estimated fee</span>
            ...
            {isConnected ? (
              ...
              <div className="text-center py-2">
                <p className="text-sm text-text-tertiary">Connect a wallet to execute this position</p>
              </div>
            )}
          </div>
        )}
        ...
        {/* Positions tab */}
        {isConnected && tab === 'positions' && smartAccountAddress && (
          <>
            <Positions
        ...
        {/* Withdraw tab */}
        {isConnected && tab === 'withdraw' && smartAccountAddress && (() => {
