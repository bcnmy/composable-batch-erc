...
pub fn spawn_anvil(
    ...
    if let Some(config) = config {
        if let Some(cpu_per_sec) = config.cpu_per_sec {
            anvil = anvil.args(vec!["--compute-units-per-second", &cpu_per_sec.to_string()]);
        }
        if let Some(memory_limit) = config.memory_limit {
            anvil = anvil.args(vec!["--memory-limit", &memory_limit.to_string()]);
        }
        if let Some(timeout) = config.timeout {
            anvil = anvil.args(vec!["--timeout", &timeout.to_string()]);
        }
    }
    if fork_optimism {
