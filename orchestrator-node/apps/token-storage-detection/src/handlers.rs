use super::state::{AppState, Chain};
    ...
    fn into_response(self) -> AxumResponse {
        match self {
            AppError::UserError(err) => match err {
                ...
                | UserError::ChainNotFound
                | UserError::ProviderNotFound => (
    ...
    H: Sync + Send + Clone + 'static,
{
    ...
    if let Some(db_conn) = &app_state.db_connection {
        ...
        if let Some(response) = response {
            return match response {
                SearchResponseWrapper::NotFound => {
                    Err(AppError::UserError(UserError::SlotNotFound))
                }
                SearchResponseWrapper::Found(entry) => {
                    Ok((Json(Response::from(entry)), InfoSource::Database))
                }
            };
        }
    }
    ...
    let trace_fn = if provider.local_tracing {
    ...
    let response = token_bss::find_slot(&provider.provider, token, None, trace_fn)
        .await
        .map_err(|err| {
            if err.to_string().contains("No valid slots found") {
                if let Some(db_conn) = &app_state.db_connection {
                    ...
                    db_conn
                        .store_search_response(&token, &chain, &response)
