...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        let chains: Vec<Chain> = Chain::iter().collect();

        // Chain & RPCs config
        let available_chains = chains;

        let chain_configs = available_chains
            .into_iter()
            .filter_map(|chain| {
        ...
        let redis_enabled = std::env::var("REDIS_ENABLED")
            ...
            .map(|s| s == "1")
            .unwrap_or(false);
        let redis_config = if redis_enabled {
            ...
            redis_config.is_tls = std::env::var("REDIS_IS_TLS")
                .ok()
                .map(|s| s == "1")
                .unwrap_or(false);
            Some(redis_config)
        ...
        // Anvil config
        let anvil_enabled = std::env::var("ANVIL_ENABLED")
            .ok()
            .map(|s| s == "1")
            .unwrap_or(false);
        let anvil_config = if anvil_enabled {
            Some(AnvilConfig {
                cpu_per_sec: std::env::var("ANVIL_CPU_PER_SEC")
                    .ok()
                    .and_then(|s| s.parse::<u32>().ok()),
                memory_limit: std::env::var("ANVIL_MEMORY_LIMIT")
                    .ok()
                    .and_then(|s| s.parse::<u32>().ok()),
                timeout: std::env::var("ANVIL_TIMEOUT_MS")
                    .ok()
                    .and_then(|s| s.parse::<u32>().ok()),
            })
        } else {
